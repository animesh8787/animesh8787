...
def render_repo(repo, theme):
    ...
    # shrink the title if a long repo name would collide with the right edge
    name = repo["name"]
    name_fs = 14.5
    avail = W - (pad + 22) - pad
    while name_fs > 10.5 and text_width(name, name_fs) > avail:
        name_fs -= 0.5
    out.append(
        f'<text x="{pad + 22}" y="{pad + 12}" font-size="{name_fs:g}" font-weight="700" '
        f'fill="{c["title"]}">{esc(name)}</text>'
    )
